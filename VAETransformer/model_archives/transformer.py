...
for n, file in tqdm(list(enumerate(files))):
  if n >= 500:
    break
  with open(file, "rb") as doc:
    d = load(doc)
    examples.append(np.expand_dims(np.concatenate([np.zeros_like(d[:1]), d, np.zeros_like(d[:1])], axis=0), 0))

train, test = train_test_split(np.concatenate(examples, axis=0))
...
class PositionalEmbedding(tf.keras.layers.Layer):
  def __init__(self, vocab_size, d_model):
    ...
    self.pos_encoding = positional_encoding(length=1290, depth=d_model)
...
if True:#with tf.device("/gpu:0"):
  transformer.fit(train_batches, epochs=100, validation_data=val_batches)